...
nyc_squirrels_colors <- nyc_squirrels %>% 
  ...
  ggplot(aes(x = long, y = lat, color = colors)) +     
  geom_point() 
```
...
## Can i remove the "no noise" tatobservation? 
# filter or drop factor levels, forcats package
nyc_squirrels <- nyc_squirrels %>%
...
```{r viz-of-position-by-squirrel-number}
nyc_squirrels %>%
...
ggplot(nyc_squirrels, aes(hectare_number, hectare_letter, fill= hectare_squirrel_number)) + 
  geom_tile() + 
  scale_fill_viridis_c(option = "plasma", direction = -1)

# Could also set up bins 
```
